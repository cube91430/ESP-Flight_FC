...
EscCal esc[4] = {
  ...
  { 1120, 1880, 0.78f }   // Cyclone – measured values (example)
};
  ...
  if (Serial.available()) {
    ...
    if (c == '\n') {
      if (input.startsWith("c ")) {
        float scale = input.substring(2).toFloat() / 100.0f;
        esc[3].scale = constrain(scale, 0.60f, 0.80f);
        Serial.print("Cyclone scale: ");
        Serial.println(esc[3].scale * 100);
      } else if (input.startsWith("f ")) {
        float boost = input.substring(2).toFloat() / 100.0f;
        // You can add front boost tuning later
        Serial.print("Front boost: ");
        Serial.println(boost * 100);
      }
      input = "";
    } else {
      input += c;
...
void loop() {
  ...
  // === SERIAL ROLL TRIM (type "r 5" = +5% left, "r -3" = -3% right) ===
  static String cmd = "";
  if (Serial.available()) {
    char c = Serial.read();
    if (c == '\n') {
      if (cmd.startsWith("r ")) {
        float trim = cmd.substring(2).toFloat() * 0.01f;  // input in %
        Serial.print("Roll trim: ");
        Serial.println(trim * 100);
        // Apply in mixer below
        static float serialRollTrim = 0.0f;
        serialRollTrim = trim;

        // Apply to motors
        MotorInput1 += serialRollTrim * 100;  // FL +
        MotorInput4 += serialRollTrim * 100;  // BL +
        MotorInput2 -= serialRollTrim * 100;  // FR -
        MotorInput3 -= serialRollTrim * 100;  // BR -
      }
      cmd = "";
    } else {
      cmd += c;
  ...
  // === ADD AFTER MIXER, BEFORE MOTOR WRITE ===
  float throttleNorm = (InputThrottle - 1148) / 684.0f;  // 0 to 1

  if (throttleNorm > 0.4f) {                  // Only at mid-high throttle
    float frontBoost = 0.12f * throttleNorm;  // +12% front
    float rearReduce = 0.06f * throttleNorm;  // -6% rear

    MotorInput1 *= (1.0f + frontBoost);
    MotorInput2 *= (1.0f + frontBoost);
    MotorInput3 *= (1.0f - rearReduce);
    MotorInput4 *= (1.0f - rearReduce);
  }
  ...
  if (InputThrottle <= ESC_PPM_MIN + 10) {
    motor1.writeMicroseconds(1148);
    motor2.writeMicroseconds(1148);
    motor3.writeMicroseconds(1148);
    motor4.writeMicroseconds(1120);
    reset_pid();
    ...
    for (int i = 0; i < 4; i++) {
      ...
      uint16_t us;
      if (i == 3) {
        // Cyclone: 1000–2000 range
        us = 1000 + (uint16_t)(norm * 1000);  // 1000 to 2000
      } else {
        // EMAX: 1148–1832 range
        us = 1148 + (uint16_t)(norm * 684);  // 684 = 1832-1148
      }

      // Apply scale (thrust match)
      us = (uint16_t)(us * esc[i].scale);

      // HARD SAFETY: Never below min, never above max
      us = max(us, esc[i].min_us);
      us = min(us, esc[i].max_us);
